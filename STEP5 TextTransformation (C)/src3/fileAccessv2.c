...
void errPrint(const char *str,...){
	...
	char c;
	char* s;
	printf("ERROR; %d; %s; ",lastLineNumber,string_attr);
	for(i=0;i<=length;i++){
		if(str[i]=='%'){
			switch (str[i+1]){
				case 'd':
...
void logPutOut(const char *str,...){
	...
	if(LogCount==0){
	    ...
	    struct tm *utc;
	    utc = localtime(&timer);
	    sprintf(fName,"./log/%s_%d%d%d-%d:%d.txt",filename,utc->tm_year+1900,utc->tm_mon + 1,utc->tm_mday,utc->tm_hour,utc->tm_min);
		fp=fopen(fName,"w");
		if(fp==NULL) exit(-1);
		for(i=0;i<length;i++){
			if(str[i]=='%'){
